...
if(isset($_POST['buscar_Pago']))
{
    ...
    $query=mysqli_query($conn,"SELECT deudas.id, deudas.id_deudor, my_user.username, FORMAT(deudas.deuda, 2), 
    deudas.fecha FROM deudas join my_user on deudas.id_deudor = my_user.id
    WHERE fecha BETWEEN '$fechaInicio' AND '$fechaFin' ORDER BY id");
...
else
{
    ...
    {
    echo '<h2>Se encontraron los siguientes pagos:</h2>'.
    "<tr><td>" . $row["id"]. "</td><td>" . $row["id_deudor"]. "</td><td>" . $row["username"]. "</td><td>"
